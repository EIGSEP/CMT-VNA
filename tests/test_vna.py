...
class TestDummyVNA:
    ...
    def test_initialization(self):
        ...
        assert self.vna.vna_ip == "127.0.0.1"
        assert self.vna.vna_port == 5025
        assert self.vna.save_dir == Path(".")
        assert self.vna.switch_nw is None
    ...
    def test_measure_osl_without_switch_network(self):
    ...
    def test_measure_osl_with_switch_network(self):
        """Test OSL measurement with switch network."""
        # Mock switch network
        mock_switch = MagicMock()
        self.vna.switch_nw = mock_switch
        self.vna.setup(1e6, 250e6, 1000, 100, -5)
        
        osl = self.vna.measure_OSL()
        
        # Verify switch network was called
        assert mock_switch.switch.call_count == 3
        mock_switch.switch.assert_any_call("VNAO")
        mock_switch.switch.assert_any_call("VNAS")
        mock_switch.switch.assert_any_call("VNAL")
        
        assert isinstance(osl, dict)
        assert len(osl) == 3
    
    def test_add_osl(self):
        """Test add_OSL method."""
        self.vna.setup(1e6, 250e6, 1000, 100, -5)
    ...
    def test_measure_ant_with_switch_network(self):
        ...
        assert mock_switch.switch.call_count == 2
        mock_switch.switch.assert_any_call("VNAANT")
        mock_switch.switch.assert_any_call("VNAN")
        ...
        assert mock_switch.switch.call_count == 1
        mock_switch.switch.assert_called_with("VNAANT")
    ...
    def test_measure_rec_with_switch_network(self):
        ...
        mock_switch.switch.assert_called_once_with("VNARF")
        assert "rec" in s11
    ...
    def test_read_data(self):
        ...
        # Test multiple measurements - may have same timestamp due to second precision
        self.vna._clear_data()
        self.vna.read_data(num_data=3)
        
        # Should have at least 1 measurement (multiple calls may overwrite due to timestamp)
        assert len(self.vna.data) >= 1
